...
@section('content')    
    <div class="ms-5 me-5">           
        ...
        @foreach($orders as $order)
            {{-- @dd($category) --}}
            <a class="link-card" href="/order/customer/{{$order->id}}">
                <div class="card mt-5">
                    <div class="row ps-3 pt-3 pb-3">
                        <div class="col-auto">
                            <img src="{{ $order->buyer->foto }}" alt="foto" style="width:160px; border-radius:20px;">
                        </div>
                        <div class="col">
                            <p style="font-weight:700; font-size:30px; margin:0em; padding:0em;">{{ $order->buyer->name }}</p>
                            <p style="font-size:20px; margin:0em; padding:0em;">Keahlian {{ $order->buyer->keahlian->nama ?? "-" }}</p>
                            <p style="font-size:20px; margin:0em; padding:0em;">Alumni {{ $order->buyer->instansi }}</p>
                            <div class="row mt-3">
                                <div class="col">
